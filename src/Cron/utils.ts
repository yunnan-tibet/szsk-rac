export const meaningSecond = (second: string) => {
  if (second === '*') {
    return '每秒钟执行';
  }
  if (second.includes('-')) {
...
export const meaningMinute = (minute: string) => {
  if (minute === '*') {
    return '每分钟';
  }
  if (minute.includes('-')) {
...
export const meaningHour = (hour: string) => {
  if (hour === '*') {
    return '每小时';
  }
  if (hour.includes('-')) {
...
export const meaningDay = (day: string) => {
  if (day === '*') {
    return '每日';
  }
  if (day === '?') {
...
export const meaningMonth = (month: string) => {
  if (month === '*') {
    return '每月';
  }
  if (month.includes('-')) {
...
export const meaningWeek = (week: string) => {
  if (week === '*') {
    return '每周';
  }
  if (week === '?') {
  ...
  return `的${week.split(',').map((key: string) => WEEK_ENUM[key])}`;
};
export const meaningYear = (year: string) => {
  if (year === '*') {
    return '每年';
  }
  if (year === '?') {
